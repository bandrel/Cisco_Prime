...
def helpmsg():
    print >> StandardError('Usage: download-configs.py [Options]>' \
...
verbose = ''
try:
    opts, args = getopt.getopt(sys.argv[1:],"u:p:c:hv",["input=", "user=", "password=", "config=","help","verbose"])
    ...
    for y in range(len(get_devices_json[u'queryResponse'][u'entity'])):
        try:
            if get_devices_json[u'queryResponse'][u'entity'][y][u'configVersionsDTO'][u'fileInfos'][u'fileInfo'][0].values()[0] == u'RUNNINGCONFIG':
                ...
                #if verbose flag is set output to stdout a message about the creation of the file
